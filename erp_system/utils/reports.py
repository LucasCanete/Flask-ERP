...
from erp_system.models import Sale, Service
...
def generate_pdf_for_date(date):
    ...
    sales = Sale.query
    sales = sales.filter(Sale.datetime >= start_datetime, Sale.datetime < end_datetime)
    ...
    try:
        with app.app_context():
            ...
            file_path = os.path.join(
                output_dir, f"reporte_{today.strftime('%d-%m-%Y')}.pdf"
            )
...
def send_pdf_daily_job():
    ...
    if pdf_path:
        ...
        logging.info("📨 PDF has been sent to telegram")
